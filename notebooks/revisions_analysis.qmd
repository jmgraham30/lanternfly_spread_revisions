...
```{r}
#| label: fig-estimates
#| fig-cap: Estimated coefficients for the linear predictors in the generalized additive model. Results are on the scale of odds ratio.


gam_mod_21_ests <- plot_model(gam_mod_21)$data %>%
  tibble() %>%
  mutate(coeff = factor(c("Interstate Presence (Yes)","Garden Centers","Population","Interstate Presence (Yes) : Garden Centers"),levels=c("Interstate Presence (Yes) : Garden Centers","Population","Garden Centers","Interstate Presence (Yes)")))
...
gam_mod_21_ests %>%
  ...
  annotate(geom = "text", x = log(3.34)+0.1,y=4.2,label="p-value=0.054") +
  annotate(geom = "text", x = log(0.276),y=3.2,label="p-value=0.041") +
  annotate(geom = "text", x = log(5.37),y=2.2,label="p-value=0.046") +
  annotate(geom = "text", x = log(7.10),y=1.2,label="p-value=0.005") +
...
results_df <- tibble(Coefficient = c("Intercept","Interstate Presence - Yes","Garden Centers","Population (log)","Interstate Presence - Yes : Garden Centers"),
